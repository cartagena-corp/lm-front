...
export default function CreateTaskForm({ onSubmit, onCancel }: FormProps) {
  ...
  const [arrayDescriptions, setArrayDescriptions] = useState<{ text: string }[]>([])
      ...
      <div className='pt-4 pb-14 space-y-2'>
        ...
        <div className='space-y-1'>
          ...
          <div className='flex justify-between items-stretch gap-2'>
            <AutoResizeTextarea onChange={(str) => setCurrentDescValue(str)}
              value={currentDescValue}
              className='text-sm'
            />
            <button type='button' onClick={() => { setArrayDescriptions([...arrayDescriptions, { text: currentDescValue }]), setCurrentDescValue("") }}
              className='bg-blue-600 hover:bg-blue-700 text-white duration-150 rounded-md border-transparent border flex justify-center items-center p-2'>
              <PlusIcon stroke={2.5} size={18} />
            </button>
          </div>
          {
            arrayDescriptions.length > 0 &&
            <div className='border-black/15 flex flex-col rounded-md border gap-2 mt-2 p-2 space-y-1 max-h-28 overflow-y-auto'>
              {
                arrayDescriptions.map((obj, i) =>
                  <div key={i} className='flex justify-start items-center gap-1'>
                    <button type='button' onClick={() => { setArrayDescriptions(arrayDescriptions.filter((_, index) => index !== i)) }}
                      ...
                      <XIcon stroke={2.5} size={14} />
                    </button>
                    <span className='line-clamp-1 text-sm'>{obj.text}</span>
                  </div>
                )
              }
            </div>
