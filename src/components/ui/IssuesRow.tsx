...
export default function IssuesRow({ spr, setIsOpen }: { spr: SprintProps, setIsOpen: Dispatch<SetStateAction<boolean>> }) {
   ...
   return (
      <>
         <Droppable id={spr.id as string} key={spr.id} styleClass={"bg-white space-y-2 rounded-md mb-6 p-6"}>
            ...
            <div className="text-sm">
...

               {
                  spr.tasks?.content.length !== 0 ?
...

                        {
                           spr.tasks?.content.map((task: TaskProps) =>
                              <Draggable key={task.id} id={task.id as string}
                                 styleClass="bg-white active:bg-sky-100 active:border-transparent active:rounded-md border-black/10 hover:bg-black/5 grid grid-cols-12 items-center border-b gap-2 px-2 py-4 
                     ...
                     : <p className="text-black/50 text-center text-sm py-2">No hay tareas disponibles</p>
               }
            </div>
